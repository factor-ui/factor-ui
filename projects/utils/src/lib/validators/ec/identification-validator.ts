...
export function identificationValidator(type: string): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    const number: string = String(control.value);
      ...
      const cedula = lengthValidator(number, 10) &&
      ...
      const ruc = ruc_natural || ruc_privada || ruc_publica;
      const id = cedula || ruc_natural || ruc_privada || ruc_publica;
      switch (type) {
        case 'cedula':
          return !cedula ? { invalidIdentification: true } : null;
          break;
        case 'ruc':
          return !ruc ? { invalidIdentification: true } : null;
          break;
        case 'id':
          return !id ? { invalidIdentification: true } : null;
          break;
        default:
          return null;
          break;
